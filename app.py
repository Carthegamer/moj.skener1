...
if btn or ticker:
    with st.spinner(f'Analiziram {ticker}...'):
        ...
        if not fin_y.empty:
            ...
            pillars = {}
            # Logika (Skraćena za preglednost, ista kao prije)
            ...
            with cg1:
                ...
                fig.update_layout(height=300, margin=dict(l=10,r=10,t=10,b=10), template="plotly_white", barmode='group')
            ...
            with m1: eps_m = st.number_input("EPS", value=5.85)
            ...
            st.metric("Sticker Price", f"${res/4:.2f}", f"MOS: ${res/8:.2f}")
